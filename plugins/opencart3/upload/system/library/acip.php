...
class Acip {
    ...
    /** Forward a single order add/status-change to the order sync webhook. */
    public function syncOrder($order, $event = 'upsert') {
        return $this->call('/v1/sync/order/webhook?source=opencart', $this->sync_key, array(
            'event' => $event,
            'order' => $order,
        ));
    }

    /** Bulk import order history (initial backfill), same shape as products. */
    public function bulkImportOrders($orders) {
        return $this->call('/v1/sync/order/bulk', $this->sync_key, array(
            'source' => 'opencart',
