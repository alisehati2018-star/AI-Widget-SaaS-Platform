...
- **Order sync** (toggle: "Sync orders to ACIP") — every order add / status
  change, from the admin panel *or* storefront checkout, is pushed to
  `/v1/sync/order/webhook` and indexed into its own Elasticsearch alias
  (`acip-orders`), with a matching **"Bulk import order history now"** button
  for the initial backfill via `/v1/sync/order/bulk`.
...
3. **Extensions → Modules → ACIP → Install** — this registers the footer
   widget-injection event and the product + order sync events automatically
   (OC3 event table, no manual wiring needed).
4. **Edit** the module:
   - **ACIP API URL** — e.g. `https://api.acip.example` (use **Test connection**
     to confirm it's reachable before saving).
   - **Widget API Key** — a *widget*-scoped key from **Dashboard → API Keys**
     on your ACIP account (tied to your plan/credit quota).
   - **Sync API Key** — a *sync*-scoped key from the same page.
   - Enable **Status**, choose whether to replace native search / inject the
     widget / sync orders.
5. Click **Bulk import catalogue now** and **Bulk import order history now**
   for the initial index.
6. **Uninstalling** the module automatically removes all six registered events
   (product edit/add/delete, order admin/storefront, footer injection) — no
   leftover hooks.

See [`docs/api-reference-fa.md`](../../docs/api-reference-fa.md) for the full
API contract (auth, request/response shapes, error codes) behind every call
this module makes.
...
upload/
  admin/
    controller/extension/module/acip.php        # settings, test-connection, events, bulk import (products+orders)
    model/extension/module/acip.php               # product/order → ACIP canonical shape
  ...
  catalog/
    controller/extension/module/acip.php          # widget injection + search proxy + storefront order event
  system/
    library/acip.php                              # API client (search/sync/bulk/order-sync/ping)
