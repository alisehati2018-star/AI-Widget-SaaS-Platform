...
class ACIP_Admin {
    ...
    public function sanitize($input) {
        return array(
            ...
            'sync_orders'    => empty($input['sync_orders']) ? 0 : 1,
        );
    }
    ...
    public function enqueue_assets($hook) {
        ...
        wp_localize_script('acip-admin', 'acipAdmin', array(
            ...
            'i18n'    => array(
                ...
                'importedOrders' => __('Imported %d orders.', 'acip-search'),
    ...
    public function render() {
        $o = wp_parse_args(get_option('acip_settings', array()), array(
            'enabled' => 0, 'api_url' => '', 'widget_key' => '', 'sync_key' => '',
            'replace_search' => 1, 'sync_orders' => 1,
        ));
        ...
        <div class="wrap acip-settings-page">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr><th><?php esc_html_e('Sync orders to ACIP', 'acip-search'); ?></th>
                        <td><label><input type="checkbox" name="acip_settings[sync_orders]" value="1"
                            <?php checked($o['sync_orders'], 1); ?>> <?php esc_html_e('Index every order (and status change) into Elasticsearch — uses the same Sync API Key', 'acip-search'); ?></label></td></tr>
                </table>
                <?php submit_button(); ?>
            </form>
            ...
            <hr>
            <h2><?php esc_html_e('Initial order-history import', 'acip-search'); ?></h2>
            <p><?php esc_html_e('Push recent orders to ACIP to build the order index.', 'acip-search'); ?></p>
            <button class="button button-secondary" id="acip-bulk-orders"><?php esc_html_e('Bulk import orders now', 'acip-search'); ?></button>
            <span id="acip-bulk-orders-result"></span>
        </div>
        <?php
    }
}
