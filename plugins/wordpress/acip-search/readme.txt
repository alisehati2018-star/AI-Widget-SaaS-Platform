...
* Optionally syncs every order add / status change to its own Elasticsearch
  index (`acip-orders`), so orders can be looked up and analysed too.
* Offers one-click bulk imports for both the catalogue and the order history.
...
4. Enable the plugin, choose whether to replace native search / sync orders,
   and **Save**.
5. Click **Bulk import now** for the initial catalogue index, and **Bulk
   import orders now** for the initial order-history index.

See `docs/api-reference-fa.md` in the platform repository for the full API
contract (auth, request/response shapes, error codes) behind every call this
plugin makes.
...
= 1.1.0 =
* Order sync: every order add / status change (admin or storefront) is pushed
  to ACIP and indexed into its own Elasticsearch alias, with a matching
  bulk order-history import.
