...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var bulkOrdersBtn = document.getElementById("acip-bulk-orders");
    var bulkOrdersOut = document.getElementById("acip-bulk-orders-result");
    if (bulkOrdersBtn && bulkOrdersOut) {
      bulkOrdersBtn.addEventListener("click", function () {
        bulkOrdersOut.textContent = acipAdmin.i18n.working;
        var body = new URLSearchParams({
          action: "acip_bulk_import_orders",
          nonce: acipAdmin.nonce,
        });
        fetch(acipAdmin.ajaxUrl, { method: "POST", body: body })
          .then(function (r) {
          ...
          .then(function (j) {
            bulkOrdersOut.textContent = j.success
              ? acipAdmin.i18n.importedOrders.replace("%d", j.data.count)
              : acipAdmin.i18n.failed;
          })
          .catch(function () {
            bulkOrdersOut.textContent = acipAdmin.i18n.failed;
          });
      });
    }
