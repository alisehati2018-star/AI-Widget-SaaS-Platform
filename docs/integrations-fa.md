...
> برای مرجع کامل API (احراز هویت، امنیت، تمام endpointها، و شکل دقیق درخواست/پاسخ
> برای محصول **و سفارش**) به [`api-reference-fa.md`](api-reference-fa.md) مراجعه کنید.
> کد منبع هر دو ماژول در [`/plugins`](../plugins/README.md) است.
...
### نصب افزونه
1. پوشهٔ `plugins/opencart3` را فشرده کنید (فایل `install.xml` + پوشهٔ `upload`) و از **Extensions → Installer** آپلود کنید.
...
### نصب افزونه
1. پوشهٔ `plugins/wordpress/acip-search` را zip کنید و از **افزونه‌ها → افزودن → بارگذاری** نصب و فعال کنید.
...
نکتهٔ حذف: مسیر pull فقط «تغییرات» را می‌بیند؛ **حذف محصول** از مسیر push (رویداد حذف افزونه → tombstone) منتشر می‌شود. اگر افزونه غیرفعال بوده و محصولی حذف شده، یک «ورود کامل کاتالوگ» از افزونه بزنید یا محصول را از داشبورد ویترین (بخش کاتالوگ) پاک‌سازی کنید.

---

## ۴) همگام‌سازی سفارش‌ها با Elasticsearch

هر دو ماژول یک کلید تنظیمات مستقل به نام **«همگام‌سازی سفارش‌ها با ACIP»** دارند
(پیش‌فرض: فعال). وقتی فعال باشد:

* هر **افزودن سفارش جدید** یا **تغییر وضعیت** آن (چه از پنل ادمین، چه از تسویه‌حساب
  فروشگاه) بلافاصله به `/v1/sync/order/webhook` پوش می‌شود و در ایندکس مجزای
  `acip-orders` می‌نشیند — با همان کلید `sync` که کاتالوگ را همگام می‌کند.
* دکمهٔ **«ورود کامل سوابق سفارش‌ها اکنون»** سفارش‌های اخیر را یک‌جا از طریق
  `/v1/sync/order/bulk` وارد می‌کند (برای backfill اولیه).
* لغو/مرجوعی/تغییر وضعیت سفارش هرگز آن را از ایندکس پاک نمی‌کند — فقط فیلد
  وضعیت به‌روز می‌شود، تا تحلیل «چند سفارش لغو شد» هم ممکن بماند. حذف واقعی
  فقط وقتی رخ می‌دهد که خودِ رکورد سفارش در فروشگاه پاک شود.

شکل دقیق payload، نگاشت فیلدها در Elasticsearch، و کدهای خطا در
[`api-reference-fa.md`](api-reference-fa.md) §۲.۳ مستند شده‌اند.

## ۵) pilot محلی بدون نصب PHP (شبیه‌ساز)
